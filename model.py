...
MODEL_ENDPOINT = "ep-20251204152915-dcjz8"
...
class TestCase(BaseModel):
    ...
    type: str = Field(description="业务逻辑 (Stream A) 或 通用标准 (Stream B)")
...
class ModulePlan(BaseModel):
    ...
    planned_stream_a_scenarios: List[str] = Field(
        description="Stream A (业务逻辑) 场景标题。每个模块至少规划 6 个。"
    )
...
async def step_1_analyze_and_plan(llm, content_parts) -> TestPlanResult:
    ...
    system_prompt = """
Role: 资深 QA 架构师 (Senior Architect)
Mission: 阅读 PRD 和 其中的ui和流程图，制定一份详尽的测试计划。
**注意：这一步只需要制定计划（测什么），不需要写具体步骤。**
...
1. **全局扫描**：识别所有功能模块。
2. **约束提取**：识别输入域 (Input Domain) 及其边界，以及核心业务规则 (Core Business Rules)。
3. **场景规划**：
    - Stream A (业务逻辑): 你应该尽可能多地覆盖 Happy Path、异常路径、逻辑冲突。**每个模块至少规划 6 个 Stream A 场景。**
    ...
    - Stream B (通用检查): 针对输入域，用于非功能/UI/安全/边界。
...
1. **逐条执行**：遍历计划中 `planned_stream_a_scenarios` 的每一个标题，生成一个 `TestCase`。
